...
static uintptr_t ScanPattern(uintptr_t start,
  uintptr_t end,
  uintptr_t length,
  uintptr_t* data,
  uintptr_t* mask)
...
uintptr_t CDynPatcher::FindPatternIDA(const std::string& pattern, uintptr_t start) const
{
  ...
  if (!start)
    start = (uintptr_t)this->DllBase;

  addr =
    ScanPattern((uintptr_t)start, reinterpret_cast<uintptr_t>(this->DllBase) +
      this->DllSize, length, data, mask);
